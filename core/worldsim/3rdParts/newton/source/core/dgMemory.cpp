...
// TOMASSINO ------------- Added to prevent a lot of Valgrind warnings (needed by memset)
#ifdef _DEBUG
#include <cstring>
#endif
// TOMASSINO -------------
...
public:
	dgGlobalAllocator ()
		:dgMemoryAllocator (__malloc__, __free__), dgList<dgMemoryAllocator*> (NULL), tommyMutex(QMutex::Recursive)
  {
    SetAllocator(this);
  }
  ...
  ~dgGlobalAllocator()
  {
    // TOMASSINO ------------- This is to prevent parent destructor from calling our methods
    removingChild = true;
    // TOMASSINO -------------
    _ASSERTE(GetCount() == 0);
  }
  ...
  // TOMASSINO ------------- Added to try to prevent locks
  QMutex tommyMutex;
  // TOMASSINO -------------
...
dgMemoryAllocator::dgMemoryAllocator()
{
  ...
  // TOMASSINO ------------- Added to try to prevent locks
  dgGlobalAllocator::m_globalAllocator.tommyMutex.lock();
  // TOMASSINO -------------
  dgGlobalAllocator::m_globalAllocator.Append(this);
  // TOMASSINO ------------- Added to try to prevent locks
  dgGlobalAllocator::m_globalAllocator.tommyMutex.unlock();
  removingChild = false;
  // TOMASSINO -------------
}
...
void *dgMemoryAllocator::MallocLow(dgInt32 workingSize, dgInt32 alignment)
{
  ...
  // TOMASSINO ------------- Added to prevent a lot of Valgrind warnings
  #ifdef _DEBUG
  memset(ptr, 0, dgUnsigned32(size));
  #endif
  // TOMASSINO -------------
...
//  return dgGlobalAllocator::m_globalAllocator.GetMemoryUsed();
  // TOMASSINO ------------- Added to try to prevent locks
  dgGlobalAllocator::m_globalAllocator.tommyMutex.lock();
  const dgInt32 m = dgGlobalAllocator::m_globalAllocator.GetMemoryUsed();
  dgGlobalAllocator::m_globalAllocator.tommyMutex.unlock();
  return m;
  // TOMASSINO -------------
}
...
//   return dgGlobalAllocator::m_globalAllocator.MallocLow(dgInt32(size));
  // TOMASSINO ------------- Added to try to prevent locks
  dgGlobalAllocator::m_globalAllocator.tommyMutex.lock();
  void* const m = dgGlobalAllocator::m_globalAllocator.MallocLow(dgInt32(size));
  dgGlobalAllocator::m_globalAllocator.tommyMutex.unlock();
  return m;
  // TOMASSINO -------------
}
...
//  return dgGlobalAllocator::m_globalAllocator.MallocLow(dgInt32(size), align);
  // TOMASSINO ------------- Added to try to prevent locks
  dgGlobalAllocator::m_globalAllocator.tommyMutex.lock();
  void* const m = dgGlobalAllocator::m_globalAllocator.MallocLow(dgInt32(size), align);
  dgGlobalAllocator::m_globalAllocator.tommyMutex.unlock();
  return m;
  // TOMASSINO -------------
}
