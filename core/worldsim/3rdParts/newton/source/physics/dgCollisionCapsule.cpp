...
dgConvexSimplexEdge dgCollisionCapsule::m_edgeArray[DG_CAPSULE_SEGMENTS
    * (6 + 8 * (DG_CAP_SEGMENTS - 1))];
    ...
    dgVector& crossInertia, dgVector& centerOfMass) const
{
  ...
  cylInertiayyzz = (dgFloat32(0.25f) * m_radius * m_radius
      + dgFloat32(1.0f / 3.0f) * m_height[0] * m_height[0]) * cylVolume;
  ...
  inertiaTensor = GetOffsetMatrix().Inverse() * inertiaTensor
      * GetOffsetMatrix();

  crossInertia.m_x = inertiaTensor[1][2]
      - volume * centerOfMass.m_y * centerOfMass.m_z;
  crossInertia.m_y = inertiaTensor[0][2]
      - volume * centerOfMass.m_z * centerOfMass.m_x;
  crossInertia.m_z = inertiaTensor[0][1]
      - volume * centerOfMass.m_x * centerOfMass.m_y;
