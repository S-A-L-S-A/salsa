...
#ifndef HILLCLIMBERALGO_H
#define HILLCLIMBERALGO_H
...
/**
 * \brief The Hill-Climber algorithm.
 *
 * The algorithm is a simplified version of a standard evolutionary algorithm
 * since it works on a population of a single individual. The individual is
 * evaluated and an offspring is derived by mutating individual's genes. The
 * offspring is then evaluated. The best individual (i.e., the one with the
 * highest fitness) will be kept for the next generation.
 *
 */
class FARSA_NEWGA_API HillClimberAlgo : public EvoAlgo
{
public:
	/**
	 * \brief Constructor.
	 ...
	 */
	HillClimberAlgo(ConfigurationParameters& params, QString prefix);
	/**
	 * \brief Destructor
	 */
	~HillClimberAlgo();
...
private:
	/**
	 * \brief Randomises the individual
	 */
	void randomiseIndividual();
	/**
